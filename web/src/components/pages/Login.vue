...
export default {
  name: 'Login',
  data: function () { return {id: '', pswd: ''} },
  props: ['modal'],
  mounted: async function () {
    var info = await eel.py_load_user_info()()
    this.id = info[0]
    this.pswd = info[1]
  },
  methods: {
    async login () {
      ...
      // ログイン処理
      var result = await eel.py_login(this.id, this.pswd)()
      if (result === 'SUCCESS') {
        this.modal.hideModal()
        Router.push('/get_image')
        return
      }
      if (result === 'ERR:LOGIN') this.modal.showModal('ログイン失敗', 'ユーザーDまたはパスワードが違います', false, {color: 'secondary', label: '閉じる'})
      if (result === 'ERR') this.modal.showModal('内部エラー', '開発者に連絡してください', false, {color: 'secondary', label: '閉じる'})
    }
  },
